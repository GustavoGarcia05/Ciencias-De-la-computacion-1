...
void generadorArreglos(int tamanio,int paso) {
	...
	//tam=sizeof(arreglo)/sizeof(*arreglo);
    //--------------------------------------------


    LARGE_INTEGER t_ini ,t_fin;
    double secs;
    cout<<"este es el tiempo para n= "<<t<<endl;
    //inicio del proceso que se desea medir
    QueryPerformanceCounter(&t_ini);
    //--------------------------------

    quickSort(arreglo,arreglo[0],arreglo[sizeof(arreglo)/sizeof(*arreglo)]);
...
void quickSort(int a[],int izq,int der){
	...
	if(der>izq){
		cout<<"aqui va entro el programa"<<endl;
		v=a[der];
		i=izq-1;
		j=der;
